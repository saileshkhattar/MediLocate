    ...
    const fetchProductAvailability = async () => {
      ...
      try {
        const res = await axios.get(
                  `http://localhost:5000/product/search/${id}`,
          ...
          <Grid container spacing={3} sx={{ mt: 2 }}>
            {[1, 2, 3].map((i) => (
              <Grid item xs={12} key={i}>
                <Skeleton variant="rectangular" width="100%" height={150} sx={{ borderRadius: '16px' }} />
              </Grid>
            ))}
          </Grid>
        </Container>
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: '#f8fafc' }}>
      ...
      >
        <Container maxWidth="lg">
          ...
          <Grid container spacing={3} alignItems="center" justifyContent={{"lg":"center"}}>
            <Grid item xs={12} md={8} mr={{"lg" :"45px"}}>
              ...
              <Typography variant="body1" sx={{ opacity: 0.8, mt: 1 }}>
                Available at {availableStores.filter(s => s.product.quantity > 0).length} nearby pharmacies
              </Typography>
            </Grid>
                ...
                <CardContent sx={{ p: 3 }}>
                  <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} md={6}>
                      <Box display="flex" alignItems="center" mb={2} sx={{minWidth : {"lg" :"400px"}}}>
                        <Avatar
                          src={store.image}
                          sx={{ width: 60, height: 60, mr: 2, bgcolor: '#f0f9ff' }}
                        >
                        ...
                        <Box>
                          <Box display="flex" alignItems="center" mb={0.5}>
                            ...
                            {store.verified && (
                              <VerifiedIcon sx={{ color: '#4caf50', ml: 1, fontSize: '1.2rem' }} />
                            )}
                          </Box>
                      ...
                      <Box display="flex" alignItems="center" mb={1}>
                        <LocationOnIcon sx={{ color: '#64748b', fontSize: '1rem', mr: 0.5 }} />
                        <Typography variant="body2" color="text.secondary">
                      ...
                      <Box display="flex" alignItems="center" gap={1}>
                        <AccessTimeIcon sx={{ color: '#64748b', fontSize: '1rem' }} />
                        <Typography variant="body2" color="text.secondary">
                          ...
                          <>
                            <Box display="flex" alignItems="center" mb={2}>
                              <Typography variant="h4" fontWeight="bold" color="#1a202c">
                                ₹{store.product.discountedPrice || store.product.price}
                              </Typography>
                              {store.product.discountedPrice && (
                                <Box ml={2}>
                                  ...
                                  <Chip
                                    label={`${getDiscountPercentage(store.product.price, store.product.discountedPrice)}% OFF`}
                                    color="success"
                                    size="small"
                                  />
                                </Box>
        ...
        {availableStores.length === 0 && (
          <Paper sx={{ p: 6, textAlign: 'center', borderRadius: '16px' }}>
            ...
            <Typography variant="body1" color="text.secondary" mb={3}>
              Try searching for similar products or contact pharmacies directly
            </Typography>
            <Button
              variant="contained"
              sx={{
